...
export default function AfricanAmHistory() {
    const [bookList, setBookList] = useState([]);
    const navigate=useNavigate()

    useEffect(() => {
        axios
            .get("http://localhost:8080/inventory/African American History")
            .then((res) => {
    ...
    return (
        <>
            <Header id="header1" message={"African American History"}></Header>
            <ul id="myList">
                {bookList.length>0?bookList.map((book, index) => {
                    ...
                    return (
                        ...
                        <div key={book._id}>
                            ...
                            <span id="description">Available: {book.avaliable?"Yes":"NO"}</span><br/>
                            <hr/>
                        </div>
