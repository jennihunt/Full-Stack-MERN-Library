...
export default function CareerReady() {
    ...
    useEffect(() => {
        axios
            .get("http://localhost:8080/inventory/Career Ready")
            .then((res) => {
    ...
    return (
        <>
            <Header id="header1" message={"Career Ready"}></Header>
            <ul id="myList">
                {bookList.length>0?bookList.map((book, index) => {
                    ...
                    return (
                        ...
                        <div key={book._id}>
                            ...
                            <button className="updatebtn" onClick={()=>handleUpdate(book._id)}>Edit Book Info</button>
                            <br/>
