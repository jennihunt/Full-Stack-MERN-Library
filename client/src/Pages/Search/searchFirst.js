...
export default function SearchFirst() {
    ...
    useEffect(() => {
        axios
            .get("http://localhost:8080/inventory/firstName")
            .then((res) => {
    ...
    function searchIt() {
        let a = bookList.filter((book) => {
            ...
            return book["authorFirst"].toUpperCase().startsWith(search.toUpperCase())
    ...
    return (
        <>
            <Header id="header1" message={"Search By Authors First Name"}></Header>
            ...
            <input id="searchInput" type="search" name="search" placeholder="Author Name" onChange={(e) => setSearch(e.target.value)} />

            <input id="searchInput" className="clickBtn" type="submit" value="^^ Search By Author Name ^^" onClick={() => searchIt()} />

            <ul id="myList">
                {searchArr ? searchArr.map((book, index) => {
                    return (
                        <div key={book._id}>
                            <span id="author">📚Author:  {book.authorFirst}, {book.authorLast}</span><br />
                            ...
                            {!book.avaliable ?
                                <span id="description"><i>Book Currently Checked Out by: </i><br /><u> {tryingit(book.bookID) != "" ? tryingit(book.bookID) : ""}</u><br /> <button id="checkOutBtn" onClick={() => addBookback(book.bookID)}>Check book back in</button></span>
                                :
