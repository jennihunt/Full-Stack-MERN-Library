...
export default function Search() {
    ...
    function searchIt() {
        let a = bookList.filter((book) => {
            ...
            return book["authorLast"].toUpperCase().startsWith(search.toUpperCase())
    ...
    const handleSubmit = (e) => {
        ...
        //if the person is found on the restricted list it dosnt let them
        if (found.length > 0) {
            setShowError(true);
        }
        else {
    ...
    return (
        <>
            <Header id="header1" message={"Search By Authors LastName"}></Header>
                ...
                {searchArr ? searchArr.map((book, index) => {
                    return (
                        ...
                        <div key={book._id}>
                            ...
                            {!book.avaliable ?
                                <span id="description"><i>Book Currently Checked Out by: </i><br /><u> {tryingit(book.bookID).length >= 1 ? tryingit(book.bookID) : ""}</u><br /> <button id="checkOutBtn" onClick={() => addBookback(book.bookID)}>Check book back in</button></span>
                                :
